...
class ShipType:
    def __init__(self, ship_type):
        ...
        elif ship_type == config.ShipType.adcp:
            self.ship_obj = Adcp()
        elif ship_type == config.ShipType.multi_draw_detect:
            self.ship_obj = MultiDrawDetect()
...
class Adcp:
    ...
    def draw(self, data_manager_obj):
        ...
        if config.home_debug:
            ...
            if data_manager_obj.server_data_obj.mqtt_send_get_obj.adcp:
                data_manager_obj.deep = round(random.random() * 100, 2)
        elif config.b_adcp_deep and data_manager_obj.pi_main_obj.deep_obj:
            deep = data_manager_obj.pi_main_obj.deep_obj.read_deep_data()
            data_manager_obj.deep = round(deep, 2)
    ...
    def send_data(self, data_manager_obj):
        ...
        if self.b_send_data and data_manager_obj.deep != -1:
            ...
            if data_manager_obj.action_id:
                deep_data.update({'planId': data_manager_obj.action_id})
            data_manager_obj.send(method='mqtt', topic='deep_data_%s' % config.ship_code, data={'deep': data_manager_obj.deep},
                                  qos=0)
            # 发送到服务器
            if len(data_manager_obj.data_define_obj.pool_code) > 0:
                try:
...
class MultiDrawDetect:
    ...
    def draw_detect(self, data_manager_obj):
        """
        水质检测抽水
        """
        # 判断开关是否需要打开或者关闭
        ...
        if config.home_debug or not config.b_draw:
            ...
            if data_manager_obj.server_data_obj.mqtt_send_get_obj.b_draw:
                ...
                # 前端发送抽水 没有采样数据为检测
                    ...
                    self.draw_detect(data_manager_obj)
            else:
                data_manager_obj.dump_draw_list = [0, 0]
            # 预先存储任务深度和水量
            if data_manager_obj.current_arriver_index == len(data_manager_obj.sort_task_done_list):
