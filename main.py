...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'baiduMap'))
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dataGetSend'))
...
def main(mod='auto'):
    ...
    # 查询改船是否注册 若未注册直接退出
    try:
        binding_data = data_manager_obj.send(method='http', data="", url='http://192.168.8.13:8009/admin/xxl/device/binding/%s'%(config.ship_code),
                              http_type='GET')
    ...
    while True:
        try:
            ...
            if mod=='auto':
                ...
                while len(data_manager_obj.server_data_obj.mqtt_send_get_obj.target_lng_lat)<=0:
                    time.sleep(2)
