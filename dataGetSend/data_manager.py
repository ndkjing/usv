...
class DataManager:
    ...
    def get_com_data(self):
        while True:
            ...
            self.logger.info({'row_com_data_read': row_com_data_read})
            com_data_read = str(row_com_data_read)[2:-5]
            self.logger.debug({'str com_data_read': com_data_read})
            ## 解析串口发送过来的数据
            ...
            try:
                ...
                self.logger.debug({'ship_current_direction': self.ship_current_direction,
                ...
                self.logger.error({'串口数据解析错误':e,'原始数据':row_com_data_read})
        ...
        try:
            while True:
                ...
                if d in [0,-1,1,2,90,180,270]:
                    manul_or_auto = 1
                # 手动模式使用用户给定角度
                self.logger.debug({'d':d,'selg.manul':self.b_manul,'count':count})
                if  manul_or_auto==1 :
                    if d == 0:
                        temp_com_data = 1
                    elif d == 90:
                    ...
                    if temp_com_data is not None :
                        if last_control is None or last_control!=temp_com_data:
                            ...
                            self.logger.info('com_data_send' + com_data_send)
                ...
                if self.b_manul==0:
                    ...
                    if self.data_define_obj.status['current_lng_lat'] is None:
                        self.logger.info('data_define_obj.status current_lng_lat is None')
                    ...
                    if self.send_target_gaode_lng_lat is None or self.send_target_gaode_lng_lat!=self.plan_path[self.current_index]:
                        ...
                        self.logger.info({'send_target_lng_lat_gps': self.send_target_gaode_lng_lat,
        ...
        while True:
            ...
            #     time.sleep(1)
                ...
                self.send(method='mqtt', topic='detect_data_%s' % (config.ship_code), data=mqtt_send_detect_data,
                          qos=1)
                self.logger.info({"发送检测数据": mqtt_send_detect_data})
                self.logger.info({"发送状态数据": mqtt_send_status_data})
