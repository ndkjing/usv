...
class Controller():
    ...
    def iteratePID(self):
        ## 获取到目标点差距
        g_theta = lng_lat_calculate.angleFromCoordinate(self.current.x, self.current.y,self.goal.x,self.goal.y)
        distance = lng_lat_calculate.distanceFromCoordinate(self.current.y, self.current.x,self.goal.x,self.goal.y)
        d_x = -distance*math.sin(g_theta)
        d_y = distance*math.cos(g_theta)
...
def main():
    ...
    targets = [State(0, 20, 0), State(20, 10, 0), State(0, 5, 0), State(-10, -15, 0), State(0, -10, 0),
               State(8, -10, 0)]
    if simulateParkour:
